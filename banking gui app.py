    ...
    for name in all_accounts:
        if name == login_name:
            ...
            if login_password == password:
                ...
                Button(account_dashboard, text="Balance",font=('Calibri',12),width=30,command=view_balance).grid(row=5,sticky=N,padx=10, pady=5)
                Button(account_dashboard, text="Statement",font=('Calibri',12),width=30,command=view_statement).grid(row=6,sticky=N,padx=10)
    ...
    if len(details) > 5:
        ...
        for transaction in transactions:
            if transaction.startswith("Deposit"):
                pdf.cell(0, 10, "Credited amount     : + " + transaction[8:] + " ", ln=True)  # Add each deposit transaction to the PDF file
            elif transaction.startswith("Withdraw"):
                pdf.cell(0, 10, "Debited amount   : - " + transaction[9:] + " ", ln=True)  # Add each withdrawal transaction to the PDF file
...
def login():
    ...
    #Button
    Button(login_screen, text="Login", command=login_session, width=15,font=('Calibri',12)).grid(row=3,column=1,sticky=W,pady=5,padx=5)
...
#Labels
Label(master, text = "Nexus Banking App", font=('Calibri',14)).grid(row=0,sticky=N,pady=10)
Label(master, text = "The most secure bank in the world!", font=('Calibri',12)).grid(row=1,sticky=N)
